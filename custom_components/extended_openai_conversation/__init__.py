...
class MistralAIAgent(conversation.AbstractConversationAgent):
    ...
    async def async_process(
        ...
        try:
            services_called, response = await self.query(user_input, messages)
        except MistralException as err:
            _LOGGER.error(err)
            intent_response = intent.IntentResponse(language=user_input.language)
            intent_response.async_set_error(
        ...
        _LOGGER.debug("Result %s", response)
    ...
    def _async_generate_prompt(self, raw_prompt: str, exposed_entities: list[dict]) -> str:
        ...
        _LOGGER.info("exposed_entities: %s", exposed_entities)
        return prompt
    ...
    async def query(
        self, user_input: conversation.ConversationInput, messages: list[ChatMessage],
        ...
        _LOGGER.info([msg.model_dump() for msg in messages])
        ...
        if '$ActionRequired' in message.content:
            for segment in self.extract_json_objects(message.content):
                try:
                    service_call: dict[str, str] = json.loads(segment)
                    ...
                    if not service or not service_call:
                        ...
                        continue
                    user = await self.hass.auth.async_get_user(user_input.context.user_id)
                    entities_to_authorize = self.get_entities_to_authorize(service_call, service_domain)
                    for entity_id in entities_to_authorize:
                        if not user.permissions.check_entity(entity_id, POLICY_CONTROL):
                            unauthorized_response = await self.generate_unauthorized_response(
                                user.name
                            )
                    ...
                    services_called.append(service_call)
    ...
    def get_entities_to_authorize(self, service_call: dict[str, str], service_domain: str) -> list[str]:
        ...
        if 'device_id' in service_call.keys():
            ...
            for entity_id, entity_entry in entity_registry.entities.items():
                if entity_entry and entity_entry.device_id == device_id:
                    ...
                    ):  # Considered a bug: service_domain not defined
        ...
        return entities_to_authorize

    async def generate_unauthorized_response(self, username: str) -> ChatMessage:
        response = await self.client.chat(
            ...
            messages=[
                ChatMessage(
                    role="user",
                    content=(
                        f"Rewrite this sentence in GlaDOS's personality. "
                        f"Do not include ANYTHING else. "
                        f"Do not include an explanation. "
                        f"Just write a sentence or two in GlaDOS's personality: "
                        f"You are not authorized to perform this task, {username}. "
